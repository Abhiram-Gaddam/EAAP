...
import { verifyAdmin, verifyUser} from '@/app/lib/utilities/auth';
...
export async function GET(request: Request, { params }: { params: Promise<{ userId: string }> }) {
  ...
  try {
    ...
    if (membershipError) {
      console.error("Supabase Fetch Error:", membershipError);
      return NextResponse.json({ error: `Database error: ${membershipError.message}` }, { status: 500 });
    }

    if (!membership) {
      return NextResponse.json({ error: 'Membership record not found in database.' }, { status: 404 });
    }
