...
//   .from('Transaction')
//   .update({ status: 'SUCCESS',paymentId: razorpay_payment_id })   
//   .eq('id', txData.id);
...
//     }
// console.log( user.userId + ": Before"     )
//   // Upsert membership
//   const { error: upsertError } = await supabase
//   .from('MembershipDetails')
//   .upsert(
//     { userId: user.userId, status: 'PENDING_APPROVAL', updatedAt: new Date().toISOString() },
//     { onConflict: 'userId' }
//   );
//   console.log( user.userId + ": After"     )
...
export async function POST(request: Request) {
  // We use verifyUser() here because it's the standard user making the payment, not an admin
  const user = await verifyUser();
  if (!user) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
